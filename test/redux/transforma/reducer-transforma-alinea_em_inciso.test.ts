...
describe('Testando a transformação de alínea em inciso', () => {
  ...
  describe('Testando a mudança da primeira alínea em inciso', () => {
    beforeEach(function () {
      const alinea = state.articulacao.artigos[1].filhos[0].filhos[0];
    ...
    it('Deveria apresentar um inciso I com 1 alínea', () => {
      expect(state.articulacao.artigos[1].filhos[0].filhos.length).to.equal(1);
      expect(state.articulacao.artigos[1].filhos[0].rotulo).to.equal('I –');
      expect(state.articulacao.artigos[1].filhos[0].filhos[0].rotulo).to.equal('a)');
    });
    it('Deveria apresentar um inciso II com o conteúdo da antiga alínea e seu antigo item transformado em alinea', () => {
      expect(state.articulacao.artigos[1].filhos[1].rotulo).to.equal('II –');
      expect(state.articulacao.artigos[1].filhos[1].texto).to.equal('texto da alinea 1 do inciso 1 do caput do artigo 2:');
      expect(state.articulacao.artigos[1].filhos[1].filhos[0].rotulo).to.equal('a)');
      expect(state.articulacao.artigos[1].filhos[1].filhos[0].texto).to.equal('texto do item 1 da alinea 1 do inciso 1 do caput do artigo 2.');
    });
    describe('Testando eventos', () => {
      it('Deveria apresentar 3 eventos', () => {
        expect(state.ui.events.length).to.equal(3);
      });
      it('Deveria apresentar a alínea e seu filho incluídos e já renumerada a alínea', () => {
        ...
        expect(incluido.elementos![0].conteudo?.texto).to.equal('texto da alinea 1 do inciso 1 do caput do artigo 2:');
        expect(incluido.elementos![1].rotulo).to.equal('a)');
        expect(incluido.elementos![1].conteudo?.texto).to.equal('texto do item 1 da alinea 1 do inciso 1 do caput do artigo 2.');
      });
      it('Deveria apresentar a alínea original e seu filho no evento de ElementoRemoved', () => {
        const removido = getEvento(state.ui.events, StateType.ElementoRemovido);
        expect(removido.elementos!.length).equal(2);
        expect(removido.elementos![0].rotulo).equal('a)');
    ...
    describe('Testando Undo/Redo', () => {
      ...
      it('Deveria apresentar novamente um inciso I com suas 2 alíneas', () => {
        expect(state.articulacao.artigos[1].filhos[0].filhos.length).to.equal(2);
        expect(state.articulacao.artigos[1].filhos[0].filhos[0].rotulo).to.equal('a)');
        expect(state.articulacao.artigos[1].filhos[0].filhos[1].rotulo).to.equal('b)');
      });
      describe('Testando eventos', () => {
        it('Deveria apresentar 3 eventos', () => {
          expect(state.ui.events.length).to.equal(3);
        ...
        it('Deveria apresentar a alínea anteriormente criada e seu filho no evento de ElementoRemoved', () => {
          const removido = getEvento(state.ui.events, StateType.ElementoRemovido);
          expect(removido.elementos!.length).equal(2);
      ...
      describe('Testando Redo', () => {
        ...
        it('Deveria apresentar um novo inciso II com uma alinea', () => {
          expect(state.articulacao.artigos[1].filhos[1].rotulo).to.equal('II –');
          expect(state.articulacao.artigos[1].filhos[1].texto).to.equal('texto da alinea 1 do inciso 1 do caput do artigo 2:');
          expect(state.articulacao.artigos[1].filhos[1].filhos[0].rotulo).to.equal('a)');
          expect(state.articulacao.artigos[1].filhos[1].filhos[0].texto).to.equal('texto do item 1 da alinea 1 do inciso 1 do caput do artigo 2.');
