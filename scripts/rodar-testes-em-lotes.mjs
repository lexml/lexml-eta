...
function rodarLote(arquivos, indice, dirCoberturaLote) {
  ...
  // Invoca o entrypoint do wtr pelo próprio node: no Windows o bin é um .cmd, que spawnSync recusa (EINVAL) sem shell.
  const binWtr = path.join(path.dirname(require.resolve('@web/test-runner')), 'bin.js');
  const resultado = spawnSync(process.execPath, [binWtr, ...args], {
    stdio: 'inherit',
