...
- **[Bug real encontrado via E2E, corrigido]** `lexml-eta.component.ts#inicializarEdicao` despachava `ABRIR_ARTICULACAO` (que agora pode popular `state.ui.alertas` com o alerta de remissão inválida) **antes** de `setProposicao`/`resetaProposicao`/`this.limparAlertas()`, que sempre zeram `ui.alertas` incondicionalmente — o alerta era apagado no mesmo ciclo em que nascia, silenciosamente (nenhum teste unitário/reducer isolado exercitava a orquestração completa desse componente). Sintoma invisível antes desta change porque nenhum caminho de abertura de documento produzia `ui.alertas` não-vazio. Corrigido reordenando `inicializarEdicao`: a chamada que carrega o documento (`this._lexmlEta!.inicializarEdicao(...)`) passou a ser a última do método, depois de todas as limpezas de alerta — dependências entre os passos intermediários (`casaLegislativa` antes de `setProposicao`; `getParlamentares`/`atualizaListaComissoes` são só chamadas de rede) foram conferidas e preservadas. Achado exclusivamente pelo spec E2E da task 3.5 (`cypress/e2e/remissao-interna/abertura-remissao-invalida-persistida.cy.ts`) — nenhum teste unitário chega a exercitar essa orquestração completa.
