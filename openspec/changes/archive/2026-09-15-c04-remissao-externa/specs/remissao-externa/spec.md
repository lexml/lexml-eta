...
O sistema SHALL exibir um popup com ações (abrir no portal de normas, editar, excluir) ao posicionar o cursor sobre um link de remissão externa, reaproveitando o mesmo componente de popup usado pela remissão interna.

#### Scenario: Cursor entra no link
- **WHEN** o usuário posiciona o cursor sobre um link de remissão externa
- **THEN** um popup aparece com o rótulo da norma (e do dispositivo, quando informado) e as ações "Abrir", "Editar" e "Excluir"

#### Scenario: Ação "Abrir" leva ao portal de normas
- **WHEN** o usuário aciona "Abrir" no popup
- **THEN** o portal de normas configurado (`urlPortalNormas`) é aberto em nova aba com a URN da norma citada, sem deep link para o dispositivo específico — o portal não suporta apontar diretamente para um dispositivo

#### Scenario: Ação "Editar" reabre o diálogo com os campos preenchidos
- **WHEN** o usuário aciona "Editar" no popup
- **THEN** o diálogo de remissão reabre na aba Externa, com a norma e o dispositivo (quando informado) pré-preenchidos
